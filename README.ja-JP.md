...
| `meta` | `key` | バックアップフォルダのハンドル（`backupDirectory`）と自動ミラー間隔（`mirrorIntervalMinutes`、既定 10 分） |

- 初回起動は「初回ガイド」が走る：最初の EventBook を作成、または（手元にあれば）古い `data.json` を取り込む。
- エクスポート / インポートは左上の EventBook メニュー内（「データとバックアップ」セクション）にある：今の本だけ、または全冊まとめて。インポートは**常に新しい EventBook として追加**され、既存の内容を絶対に上書きしない。履歴パネルのツールバーにも「⬇️ 全ブックを出力」のショートカットがある：実装は同じ一つで、フォルダーミラーに依存せずどのブラウザでも使える。
...
履歴パネルでフォルダを選択できる（Chrome / Edge の File System Access API が必要）。設定後は各スナップショットがファイルとしても書き出される。同じパネルで確認・変更もできるが、本文は状態の 1 行だけ（接続先のフォルダと最後の同期）です。保存先やバージョン数、各ボタンは何をするのかはタイトルと各ボタンのツールチップに入っています。「🔄 全バージョンを再ミラー」は本文から外し、「📂 ミラーフォルダーを開く」パネルの下端に畳みました（Web ページからエクスプローラーは開けないので、そのパネルがフォルダ内のファイル名をそのまま一覧します。名前だけを読み、内容は読みません）。接続中は、最新コピーを書き直す間隔を 1 / 5 / 10 / 15 / 30 / 60 分から選べます（既定 10 分、この端末用の設定。「💾 いま保存する」と「全バージョンを再ミラー」は常に即時で、バージョンファイルはアーカイブのたびに必ず書き込まれます）。
...
このフォルダはあなたのもの（外付け HDD、クラウド同期、NAS なんでも可）。ブラウザのデータを消去しても、全てを失うわけではない。書き込み専用です（「ミラーフォルダーを開く」もファイル名を一覧するだけで、内容を読み返さず復元元にも使わない）：フォルダを消してもアプリの動作には影響しない。「切断」はミラーをやめるだけで、すでに書き出したコピーはそのまま残る。フォルダを交換すると全冊の履歴をまとめて新しい場所へ送り直すので、「できたはずなのに空っぽ」にはならない。
...
npm run smoke     # headless Chrome エンドツーエンド 17 ステップ（実際のマウスドラッグ + IndexedDB 直接読み取り + ネットワーク指紋）
