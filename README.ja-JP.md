...
履歴パネルでフォルダを選択できる（Chrome / Edge の File System Access API が必要）。設定後は各スナップショットがファイルとしても書き出される。同じパネルで確認・変更もできるが、本文は状態の 1 行だけ（接続先のフォルダと最後の同期）です。保存先やバージョン数、各ボタンは何をするのかは、タイトルと「🔄 全バージョンを再ミラー」「📁 フォルダーを変更」「再許可する」「接続を解除」のツールチップに入っています。接続中は、最新コピーを書き直す間隔を 1 / 5 / 10 / 15 / 30 / 60 分から選べます（既定 10 分、この端末用の設定。「💾 いま保存」と「全バージョンを再ミラー」は常に即時で、バージョンファイルはアーカイブのたびに必ず書き込まれます）。
