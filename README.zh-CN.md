...
历史面板里可以选一个文件夹（需要 Chrome / Edge 的 File System Access API），之后每份快照都会同步落成文件。同一个面板也能查看和修改它，但正文只留一行状态（连的是哪个文件夹、上次镜像到什么时候）：落盘位置、本簿有多少个版本、每个按钮到底做了什么，全放在标题和「🔄 重新镜像全部版本」「📁 更换文件夹」「重新授权」「断开」的鼠标提示里。已连接时还能选自动重写最新副本的节奏：1 / 5 / 10 / 15 / 30 / 60 分钟，默认 10 分钟（这是本设备的设置；「💾 立即存档」和「重新镜像全部版本」永远立刻写，历史版本文件也照旧随每次存档落盘）。
