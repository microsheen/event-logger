...
历史面板里可以选一个文件夹（需要 Chrome / Edge 的 File System Access API），之后每份快照都会同步落成文件。同一个面板也能查看和修改它，但正文只留一行状态（连的是哪个文件夹、上次镜像到什么时候）：落盘位置、本簿有多少个版本、每个按钮到底做了什么，全放在标题和每个按钮的鼠标提示里。「🔄 重新镜像全部版本」不再摆在正文，它收在「📂 查看镜像文件夹」面板的底部 —— 网页没法替你打开资源管理器，所以那个面板直接把文件夹里真实存在的文件名列出来（只列名字，不读内容）。已连接时还能选自动重写最新副本的节奏：1 / 5 / 10 / 15 / 30 / 60 分钟，默认 10 分钟（这是本设备的设置；「💾 立即存档」和「重新镜像全部版本」永远立刻写，历史版本文件也照旧随每次存档落盘）。
...
这个文件夹是你自己的（移动硬盘、坚果云、NAS 都行），因此"浏览器被清了"也不等于全丢。它只写不读（「查看镜像文件夹」也只列文件名，不读内容、也不回填应用状态）：删掉文件夹不会影响应用运行。在面板里点"断开"也只是不再镜像，磁盘上已经写出去的副本一个都不动；换文件夹时会把所有 EventBook 的全部历史版本整体回补到新位置，不会留下一个"看着成功、其实是空的"目录。
